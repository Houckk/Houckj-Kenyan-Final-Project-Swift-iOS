    ...
    func updateAverageRating(completed: @escaping() -> ()){
        ...
        reviewsRef.getDocuments{ (querySnapshot, error) in
            guard error == nil else {
                print("^^^^^^ ERROR: Failed to get query snapshot of review for reviewsRef: \(reviewsRef.path), error: \(error?.localizedDescription)")
                return completed()
            }
            var ratingTotal = 0.0
            ...
            spotRef.setData(dataToSave){ error in //save it and check errors
                guard error == nil else{
                    print("#### ERROR: Updating document \(self.documentID) in spot after changing averageReivew, error: \(error?.localizedDescription)")
                    return completed()
                }
